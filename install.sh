...
CID_DIR=$HOME/.chainid
while [ $# -gt 0 ]; do
  case "$1" in
    -d | --dest-dir ) CID_DIR="$2"; shift;;
  esac
  shift
done

# Check if CID_DIR exists
if [ ! -d "$CID_DIR" ]; then
  mkdir -p $CID_DIR
fi
...
NPM=$CID_DIR/node/bin/npm
NODE=$CID_DIR/node/bin/node

export TMP=$CID_DIR/tmp
export TMPDIR=$TMP
...
PATH="$CID_DIR/node/bin/:$CID_DIR/node_modules/.bin:$PATH"
...
start() {
  ...
  case $1 in
    ...
    "install" )
      ...
      # finalize
      pushd "$CID_DIR"/node_modules/.bin
      for FILE in "$CID_DIR"/node_modules/.bin/*; do
      ...
      echo $VERSION > "$CID_DIR"/installed
      echo :Done.
    ;;
...
check_python() {
  ...
  if [[ $PYTHONVERSION != "2.7" ]]; then
    echo "Python version 2.7 is required to install pty.js. Please install python 2.7 and try again. You can find more information on how to install Python in the docs: https://docs.chainid.io/ssh_workspaces.html"
    exit 100
  fi
}
...
ensure_local_gyp() {
  ...
  if [ `"$PYTHON" -c 'import gyp; print gyp.__file__' 2> /dev/null` ]; then
    ...
    if has virtualenv; then
      virtualenv -p python2 "$CID_DIR/python"
    else
      downlaod_virtualenv
      "$PYTHON" virtualenv/virtualenv.py "$CID_DIR/python"
    fi
    if [[ -f "$CID_DIR/python/bin/python2" ]]; then
      PYTHON="$CID_DIR/python/bin/python2"
    else
  ...
  local GYP_PATH=$CID_DIR/node/lib/node_modules/npm/node_modules/node-gyp/bin/node-gyp.js
  if [ -f  "$GYP_PATH" ]; then
    ln -s "$GYP_PATH" "$CID_DIR"/node/bin/node-gyp &> /dev/null || :
  fi
}
...
node(){
  ...
  # use local npm cache
  "$NPM" config -g set cache  "$CID_DIR/tmp/.npm"
  ensure_local_gyp
...
compile_tmux(){
  ...
  ./configure --disable-shared --prefix="$CID_DIR/local"
  echo ":Compiling Libevent"
  make
  ...
  CPPFLAGS=-P ./configure --prefix="$CID_DIR/local" --without-tests --without-cxx
  echo ":Compiling Ncurses"
  make
  ...
  ./configure CFLAGS="-I$CID_DIR/local/include -I$CID_DIR/local/include/ncurses" LDFLAGS="-static-libgcc -L$CID_DIR/local/lib" --prefix="$CID_DIR/local"
  echo ":Compiling Tmux"
  make
...
tmux_download(){
  ...
  # $DOWNLOAD https://raw.githubusercontent.com/chainidio/install/master/packages/tmux/libevent-2.0.21-stable.tar.gz
  ...
  # $DOWNLOAD https://raw.githubusercontent.com/chainidio/install/master/packages/tmux/ncurses-5.9.tar.gz
  $DOWNLOAD http://ftp.gnu.org/gnu/ncurses/ncurses-6.0.tar.gz
  echo ":Downloading Tmux..."
  # $DOWNLOAD https://raw.githubusercontent.com/chainidio/install/master/packages/tmux/tmux-1.9.tar.gz
...
tmux_install(){
  echo :Installing TMUX
  mkdir -p "$CID_DIR/bin"
  if check_tmux_version $CID_DIR/bin/tmux; then
  ...
  # If we can support tmux 1.9 or detect upgrades, the following would work:
    ...
    ln -sf $(which tmux) "$CID_DIR"/bin/tmux
    return 0
  ...
  # If tmux is not present or at the wrong version, we will install it
    ...
    # Linux
      ...
      tmux_download  
      compile_tmux
      ln -sf "$CID_DIR"/local/bin/tmux "$CID_DIR"/bin/tmux
    fi
  fi
...
collab(){
  ...
  mkdir -p "$CID_DIR"/lib
  cd "$CID_DIR"/lib
  $DOWNLOAD https://raw.githubusercontent.com/chainidio/install/master/packages/sqlite3/linux/sqlite3.tar.gz
  tar xzf sqlite3.tar.gz
  rm sqlite3.tar.gz
  ln -sf "$CID_DIR"/lib/sqlite3/sqlite3 "$CID_DIR"/bin/sqlite3
}

nak(){
  echo :Installing Nak
  "$NPM" install https://github.com/chainidio/nak
}
...
ptyjs(){
  ...
  if [ "$arch" == "x64" ] && [ "$os" == "linux" ] ; then
    $DOWNLOAD https://github.com/chainidio/install/releases/download/bin/pty-$NODE_VERSION-$os-$arch.tar.gz \
...
buildPty() {
  ...
  if ! hasPty; then
    ...
    echo `"$CID_DIR/node/bin/node" -e "console.log(require('pty.js'))"`
    exit 100
  fi
}

hasPty() {
  local HASPTY=`"$CID_DIR/node/bin/node" -p "typeof require('pty.js').createTerminal=='function'" 2> /dev/null ` 
  if [ "$HASPTY" != true ]; then
    return 1
  fi
}
...
rhc(){
 echo :Installing RedHat OpenShift
  "$NPM" install  rhc
}
